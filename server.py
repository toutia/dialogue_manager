...
from riva_clients.chatbot.chatbots_multiconversations_management import create_chatbot, get_new_user_conversation_index, get_chatbot
...
sio = SocketIO(app, logger=True, ping_timeout=10, ping_interval=5)
verbose = client_config['VERBOSE']
VA_INIT_MESSAGE = "Hi, welcome to DRIMVISION object finder  service. How can I help you?"
...
def connect():
    if verbose:
        ...
        emit('connect_ack',  {'key':'value'}, broadcast=False)
        print('connect ack sent')
...
if __name__=='__main__':
    sio.run(app, host='localhost', port=5555, debug=True)